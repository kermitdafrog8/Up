...
void MainForm::OnCopyToLocalDiskClick( void )
{
    ...
    if (size == 0)
        return;
    else if (size == 1 && ui.fileSystemTree->selectedItems().at(0)->text(2).left(6) != QString::fromLatin1("Folder"))
    {
    ...
    else
    {
        ...
        s = qfd.getExistingDirectory(this, QString::fromLatin1("Select Directory to Save Files"));
    }
...
QTreeWidgetItem *MainForm::AddFile(QTreeWidgetItem* Item, File *f, Drive *device)
{
    ...
    QDateTime ModifiedDate = f->DateModified;
    fItem->setText(1, ModifiedDate.toString());
    fItem->setText(2, QString::fromLatin1("File"));
...
void MainForm::PopulateTreeItems(QTreeWidgetItem *Item, bool expand)
{
    ...
    for (int i = 0; i < Folders; i++)
    {
        ...
        else
        {
            ...
            {
                if (Item->child(x)->text(0) == QString::fromLatin1(sub->Dirent.Name))
                {
...
void MainForm::OnLoadDevicesClick( void )
{
    ...
    for (int i = 0; i < (int)Drives.size(); i++)
    {
        ...
        item->setText(2, QString::fromLatin1(Type));     									// Drive type
        item->setText(3, QString::fromLatin1(current->FriendlySize.c_str()));                // Drive size
        ...
        for (int j = 0; j < (int)current->Partitions().size(); j++)
        {
            ...
            partition->setText(0, QString::fromLatin1(currentVolume.c_str()));	// Partition name
            partition->setText(2, QString::fromLatin1("Partition"));
            partition->setText(3, QString::fromLatin1(Helpers::ConvertToFriendlySize((INT64)current->PartitionGetLength(currentVolume)).c_str()));
